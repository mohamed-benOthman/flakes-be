...
export class MaquilleuseController {
  ...
  @ApiOperation({
    title:
      "Recherche artist par Departement(type=1) /par ville (type=2) / par business(type=3) / par expertise type=4 / type=5 par username / type=6 par email / All (type=0)) ",
  })
  @ApiResponse({ status: 200, description: "Maquilleuse trouvé." })
  @ApiResponse({ status: 404, description: "Pas de maquilleuse trouvé." })
  public async getAllMaquilleuse(
    @Param("parametre") parametre: string,
    @Param("type") type: string,
    @Param("debut") debt: string,
    @Param("nombre") nombre: string
  ) {
    try {
      const res = await this.maquilleuseService.findMaquilleuseTypeAll(
        parametre,
        type,
        Number(debt),
        Number(nombre)
  ...
  public async getAllMaquilleuses() {
    try {
      console.log(await this.maquilleuseService.findAllMaquilleuses());
  ...
  @ApiOperation({
    title:
      "Retourne Nbr artist par Departement(type=1) /par ville (type=2) " +
  ...
  @ApiResponse({ status: 200, description: "Maquilleuse trouvé." })
  @ApiResponse({ status: 404, description: "Pas de maquilleuse trouvé." })
  public async getNbrMaquilleuseCriteria(
    @Param("parametre") parametre: string,
    @Param("type") type: string
  ) {
    ...
    switch (type) {
      case "U":
        return await this.maquilleuseService.isMaquilleuseExisted(
          parametre,
          type
        );
        break;
    ...
    try {
      console.log("dddd");
      console.log(
        await this.maquilleuseService.findAllMaquilleuseNbr(parametre, type)
      );
    } catch (e) {
      console.log(e);
    }
  ...
  public async create(@Body() createMaquilleuse: CreateMaquilleuseDto) {
    ...
    return await this.maquilleuseService
      .createUser(createMaquilleuse, false)
      .catch((err) => {
        console.error(err);
      });
    return [];
  ...
  public async modify(@Body() modifyMaquilleuse: CreateMaquilleuseDto) {
    return await this.maquilleuseService
      .createUser(modifyMaquilleuse, true)
      .catch((err) => {
        console.error(err);
      });
    return [];
  ...
  public async loginPay(@Req() req: Request) {
    ...
    if (req.body.vads_trans_status === "ACCEPTED") {
      ...
      console.log(payment);
      const maq = await Maquilleuse.findMaquilleuseByEmailPayment(
        req.body.vads_cust_email
      );
      console.log(maq);
