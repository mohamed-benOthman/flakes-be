...
export class UserController {
  ...
  public async resetPasswordController(@Body() body: any) {
    const newVar = await this.userService.resetPassword(
      body.token,
      body.password
    );
    return newVar;
  ...
  public async resetpassword(
    ...
    res.redirect(
      "http://82.165.253.223:3050/confirm/forgotpass.html?eso=" + code
    );
    //   return result;
  }
...
*/
    ...
    if (match) {
      ...
      return { user };
    } else throw new HttpException({ passFailed }, 400);
  }
  ...
  public async gettoken(@Body() loginUserDto: LogUserDto): Promise<UserRO> {
    ...
    console.log(
      "pass:" +
        ...
        loginUserDto.password +
        " bcrypt:" +
        ToolService.getBCryptHash(loginUserDto.password)
    );
